...
harness, which compares an identical extracted-fact array against the
TypeScript compiler's own parser at 5.9.3 — the last release that still ships
the JavaScript compiler API. Medians of three independent runs: **3.13x** (Node)
and **4.29x** (Bun) on a 3,314-byte file, falling to **1.03x** and **1.27x** on
a single 841,814-byte file, where the JSON boundary rather than parsing
